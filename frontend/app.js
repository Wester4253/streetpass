...
async function loadProfile() {
    try {
        const res = await fetch('http://localhost:8010/api/profile', {
        ...
        `;
        document.getElementById('avatar-preview').src = `http://localhost:8010/api/avatar/${profile.id}`;
    } catch (err) {
...
async function uploadAvatar() {
    ...
    try {
        const res = await fetch('http://localhost:8010/api/upload-avatar', {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${state.token}` },
            body: formData
        });
...
async function showQR() {
    ...
    qrDiv.innerHTML = `<img src="http://localhost:8010/api/qr/${state.userId}">`;
}
...
async function addFriend(friendId) {
    try {
        const res = await fetch('http://localhost:8010/api/add-friend', {
            method: 'POST',
            headers: {
...
async function loadFriends() {
    try {
        const res = await fetch('http://localhost:8010/api/friends', {
            headers: { 'Authorization': `Bearer ${state.token}` }
        });
        ...
        list.innerHTML = friends.map(friend => `
            <div class="friend-card">
                <img src="http://localhost:8010/api/avatar/${friend.id}" class="avatar">
